...
# from  torch.nn imprt BatchNorm2d
...
class ResNet(nn.Module):  # renet网络的构成部分
    ...
    def _load_pretrained_model(self):
        ...
        pretrain_dict = torch.load('./network/resnet101-5d3b4d8f.pth')
        # print(pretrain_dict.keys())
        del pretrain_dict['conv1.weight']
        model_dict = {}
...
class DeepLabv3_plus(nn.Module):
    def __init__(self, nInputChannels=3, n_classes=21, os=16, pretrained=False, freeze_bn=False, _print=True):
        ...
        # 四个不同带洞卷积的设置，获取不同感受野
        self.aspp1 = ASPP_module(2048+1, 256, dilation=dilations[0])
        self.aspp2 = ASPP_module(2048+1, 256, dilation=dilations[1])
        self.aspp3 = ASPP_module(2048+1, 256, dilation=dilations[2])
        self.aspp4 = ASPP_module(2048+1, 256, dilation=dilations[3])
        self.relu = nn.ReLU()

        # 全局平均池化层的设置
        self.global_avg_pool = nn.Sequential(nn.AdaptiveAvgPool2d((1, 1)),
                                             nn.Conv2d(2048+1, 256, 1, stride=1, bias=False),
                                             BatchNorm2d(256),
                                             nn.ReLU())
    ...
    def forward(self, input, refer_map):
        # 第一次迭代
        x, low_level_features = self.resnet_features(torch.cat((input, refer_map), dim=1))
        # x, low_level_features = self.resnet_features(torch.cat((input, torch.zeros_like(refer_map)), dim=1))

        x = torch.cat((x, F.interpolate(refer_map, size=x.size()[2:], mode='bilinear', align_corners=True)), dim=1)
        # x = torch.cat((x, F.interpolate(torch.zeros_like(refer_map), size=x.size()[2:], mode='bilinear', align_corners=True)), dim=1)
