...
# class VOC12ClsDatasetMSF_DPV3(VOC12ClsDataset):
#
#     def __init__(self, img_name_list_path, voc12_root, scales, inter_transform=None, unit=1):
#         super().__init__(img_name_list_path, voc12_root, transform=None)
#         self.scales = scales
...
class CityspaceDataset(Dataset):
    ...
    def __init__(self, img_name_list_path, data_root, transform=None):
        ...
        with open(img_name_list_path, 'r') as f:
            ...
            self.img_name_list = []
            for index,dect in enumerate(jsonObj):
                self.img_name_list.append(CityspaceLabelData(dect['data'],dect['label']))
    ...
    def __getitem__(self, idx):
        ...
        # print(img_obj)
        img = PIL.Image.open(os.path.join(self.data_root,img_obj.data)).convert("RGB")
...
class CityspaceClsDataset(CityspaceDataset):
