...
class RandomResizeTensorFloat():

    def __init__(self, min_long, max_long, p=0.5):
        self.min_long = min_long
        self.max_long = max_long
        self.p = p
    ...
    def __call__(self, img):
        ...
        img = torchvision.transforms.Resize(target_shape)(img)
        return img
...
from torchvision.transforms.functional import InterpolationMode as InterpolationMode
class RandomResizeCrop():
    ...
    def __init__(self, crop_size, min_scale, max_scale):
        ...
        self.croper = torchvision.transforms.RandomCrop(size=(crop_size, crop_size), pad_if_needed=True, padding=0, padding_mode='constant')
    ...
    def __call__(self, concat):
        ...
        w = int(w * scale)
        concat = torch.concat((
            tensor_resize(concat[0:3, :, :], size=[h, w], interpolation= InterpolationMode.BILINEAR),
            tensor_resize(concat[3:c, :, :], size=[h, w], interpolation= InterpolationMode.NEAREST)
        ), dim=0)
        return self.croper(concat)